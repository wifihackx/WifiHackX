    ...
    const setupAuthListeners = (attempt = 0) => {
      ...
      `;
        ...
        const isApiKeyReferrerBlocked = error => {
            const text = `${(error && error.message) || ''} ${(error && error.customData && error.customData._serverResponse) || ''}`
                .toLowerCase()
                .trim();
            return (
        ...
        const notifyApiKeyReferrerFix = () => {
            const authDomain =
                (window.RUNTIME_CONFIG && window.RUNTIME_CONFIG.firebase && window.RUNTIME_CONFIG.firebase.authDomain) || 'white-caster-466401-g0.firebaseapp.com';
            notify(
        ...
        const ensureAppCheckForAuth = async () => {
            try {
                ...
                if (status.disabled) {
                    ...
                    if (isLocalDevHost()) {
                        Logger.info('Localhost detected: Allowing auth attempt despite App Check being disabled.', 'AUTH');
                        return true;
                    }
                    return false;
                }
