...
const shouldSkipUserLocationUpdate = () => {
  ...
  const LOGIN_SUBMIT_SELECTOR = 'button[data-testid="login-submit"], button[type="submit"]';
  ...
  let globalAuthBootstrapInitialized = false;
  let globalAuthBootstrapWaiting = false;
  let authUiHydrated = false;
  const authNoticeState = new Map();
  let googleAuthInProgress = false;
  let handleGoogleAuth = null;
  ...
  const clearDisallowedModal = viewId => {
    ...
    if (!isAdminView && adminOnlyModals.has(activeModal)) {
      ...
      Logger.debug(`Modal admin bloqueado en vista pública: ${activeModal}`, 'AUTH');
    }
  };

  const setDisplaySafe = (element, value) => {
    if (!element) return;
    if (value === '' || value === null || value === undefined) {
      // Defensive unhide to avoid stale hidden state after auth/view races.
  ...
  const enforceShellVisibilityForView = viewId => {
    ...
    const shouldHideShell = viewId === 'loginView';

    setDisplaySafe(header, shouldHideShell ? 'none' : '');
    setDisplaySafe(footer, shouldHideShell ? 'none' : '');
    ...
    if (!isMobile) {
      ...
      if (document.body) document.body.classList.remove('mobile-menu-open-body');
      if (toggle) toggle.setAttribute('aria-expanded', 'false');
      if (panel) panel.setAttribute('aria-hidden', 'false');
      if (overlay) overlay.hidden = true;
    } else if (panel && !(header && header.classList.contains('mobile-menu-open'))) {
  ...
  const syncViewState = viewId => {
    ...
    enforceShellVisibilityForView(viewId);
    try {
  ...
  const updateAdminButtons = isAdmin => {
  ...
  const syncAuthUiState = async user => {
    ...
    updateLoginButton(user || null);

    try {
      if (user) {
        const displayName = user.displayName || user.email || user.name || 'Usuario';
        localStorage.setItem('wifihackx:auth:last_display', displayName);
      } else {
    ...
    try {
      ...
      const nextAdminState = cachedAdmin || isAdminResolved;
      updateAdminButtons(nextAdminState);
      AppState.setState('user', {
        ...current,
        uid: user.uid || current.uid || null,
        email: user.email || current.email || null,
        ...
        role: nextAdminState ? 'admin' : 'client',
      });
    } catch (error) {
      Logger.debug('Could not resolve admin state during auth UI sync', 'AUTH', error);
  ...
  const restoreHeaderFooter = contextLabel => {
    ...
    const footer = document.querySelector('.modern-footer');
    if (header) {
      setDisplaySafe(header, '');
      Logger.debug(`Header restored${contextLabel ? ` (${contextLabel})` : ''}`, 'AUTH');
    }
    if (footer) {
      setDisplaySafe(footer, '');
      Logger.debug(`Footer restored${contextLabel ? ` (${contextLabel})` : ''}`, 'AUTH');
    }
    enforceShellVisibilityForView('homeView');
  };
  ...
  const ensureLocalPersistence = async () => {
    try {
      ...
      Logger.debug('Could not set auth persistence to LOCAL', 'AUTH', _error);
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < callableNames.length; i += 1) {
        ...
        try {
          ...
          return !!(result && result.data && result.data.allowed);
        } catch (error) {
          lastError = error;
      ...
      const code = String((lastError && lastError.code) || '').toLowerCase();
      const reason = String((lastError && lastError.details && lastError.details.reason) || '');
      ...
      if (code.includes('invalid-argument')) {
        notify(
          reason === 'invalid_email' ? 'Email inválido.' : 'Email no permitido para registro.',
          'warning'
        );
        return false;
      }
      ...
      notify('No se pudo validar el registro ahora. Intenta de nuevo en unos segundos.', 'warning');
      return false;
    } catch (_error) {
      Logger.warn('Error en guard de registro', 'AUTH', _error);
      notify('No se pudo validar el registro ahora. Intenta de nuevo en unos segundos.', 'warning');
      return false;
    }
  };
  ...
  const waitForLoginTemplateAndRetry = () => {
    ...
    let pollAttempts = 0;
    const MAX_POLL_ATTEMPTS = 60; // ~30s (interval 500ms)
    ...
    loginTemplatePollTimer = setInterval(() => {
      retryIfPresent();
      if (pollAttempts >= MAX_POLL_ATTEMPTS) {
        cleanupWaiters();
        loginTemplateWaiting = false;
        Logger.warn(
          'Timeout waiting for loginView template mount. Will retry on next login intent.',
          'AUTH'
        );
      }
    }, 500);
  ...
  const runGlobalAuthBootstrap = () => {
    if (globalAuthBootstrapInitialized) return;

    const firebaseReady = globalThis.firebase && typeof globalThis.firebase.auth === 'function';

    if (!firebaseReady) {
      if (!globalAuthBootstrapWaiting) {
        globalAuthBootstrapWaiting = true;
        Logger.warn(
          'Firebase aun no esta listo para auth bootstrap. Esperando firebaseReady...',
          'AUTH'
        );

        const retryBootstrap = () => {
          globalAuthBootstrapWaiting = false;
          runGlobalAuthBootstrap();
        };

        window.addEventListener('firebaseReady', retryBootstrap, { once: true });
        setTimeout(retryBootstrap, 3000);
      }
      return;
    }

    globalAuthBootstrapInitialized = true;
  ...
  const setupAuthListeners = (attempt = 0) => {
    ...
    const googleRegisterBtns = document.querySelectorAll('[data-action="registerWithGoogle"]');
    ...
    const bypassLocalRecaptcha = !!window.__WFX_RECAPTCHA_LOCAL_BYPASS__;
    const shouldAttachRecaptcha = !!recaptchaSiteKey && !(isLocalhost && bypassLocalRecaptcha);
    document.querySelectorAll('[data-callback="onRecaptchaRegisterSuccess"]').forEach(btn => {
      if (shouldAttachRecaptcha) {
    ...
    const handleLoginSuccess = async userCredential => {
      ...
      try {
        ...
        AppState.setState('user', {
          ...
          displayName: userCredential.user.displayName || userCredential.user.email || 'Usuario',
          photoURL: userCredential.user.photoURL || null,
          isAuthenticated: true,
          isAdmin: cachedAdmin,
          role: cachedAdmin ? 'admin' : 'client',
        });
        updateAdminButtons(cachedAdmin);
        resolveAdminStatus(userCredential.user)
          .then(isAdminAtLogin => {
            const next = AppState.getState('user') || {};
            AppState.setState('user', {
              ...next,
              isAuthenticated: true,
            ...
            updateAdminButtons(isAdminAtLogin);
          })
    ...
    const shouldPreferRedirectForGoogleAuth = () => {
      try {
        ...
        return isLocalDevHost() && /firefox/i.test(ua);
      } catch (_e) {
        return false;
      }
    };
    ...
    const isApiKeyReferrerBlocked = error => {
      const text =
        `${(error && error.message) || ''} ${(error && error.customData && error.customData._serverResponse) || ''}`
          .toLowerCase()
          .trim();
      return (
    ...
    const handleAuthInfraError = error => {
      const errCode = String((error && error.code) || '').toLowerCase();
    ...
    // 1. Manejo de Login
    if (loginForm) {
      const loginSubmitBtn = loginForm.querySelector(LOGIN_SUBMIT_SELECTOR);
        ...
        loginForm.addEventListener(
          ...
          async e => {
            ...
            if (googleAuthInProgress) {
              ...
              // Timeout safety
              if (
                globalThis._googleAuthStartTime &&
                Date.now() - globalThis._googleAuthStartTime > 30000
              ) {
            ...
            if (
              (submitter &&
                submitter.dataset &&
                (submitter.dataset.action === 'loginWithGoogle' ||
                  submitter.dataset.action === 'registerWithGoogle')) ||
              (target &&
                target.closest(
                  '[data-action="loginWithGoogle"], [data-action="registerWithGoogle"]'
                ))
            ) {
            ...
            const loginTabActive = loginTabPanel
              ...
              : true;
            if (!loginTabActive) {
              Logger.debug('Login form submit continuing despite inactive tab attributes', 'AUTH');
            }
            ...
            try {
              ...
              if (!loginOk) {
                ...
                notify('No se pudo completar el inicio de sesión con esta cuenta.', 'warning');
              }
            } catch (error) {
              ...
              if (!handled) {
                if (
                  ...
                  const resetSent = await offerPasswordResetOnInvalidCredential(email);
                  if (resetSent) {
                    return;
                  }
                }
          ...
          try {
            ...
            try {
              ...
              if (currentUser) {
                ...
                notify('¡Cuenta creada exitosamente! Por favor verifica tu email.', 'success');
              } else {
            ...
            Logger.debug('Navigating to home view after registration...', 'AUTH');
            showHomeViewSafe();
    ...
    handleGoogleAuth = async (arg1, arg2) => {
      const e =
      ...
      try {
        ...
        if (shouldPreferRedirectForGoogleAuth()) {
          Logger.warn(
            'Firefox en localhost detectado: usando signInWithRedirect para evitar bloqueos de almacenamiento de terceros.',
            'AUTH'
          );
        ...
        resolveAdminStatus(result.user)
          .then(isAdminAtLogin => {
            try {
              const prev = AppState.getState('user') || {};
              AppState.setState('user', {
          ...
          .catch(syncError => {
            Logger.debug('Could not resolve admin state after Google login', 'AUTH', syncError);
          });
        ...
        if (
          ...
          // Si es internal error, preguntar al usuario si la pestaña está activa.
          // Si el diálogo no puede mostrarse (tab inactiva), forzar redirect automático.
          if (error.code === 'auth/internal-error') {
            const canPromptUser =
              typeof window.confirm === 'function' &&
              document.visibilityState === 'visible' &&
              document.hasFocus();
            const shouldUseRedirect = canPromptUser
              ? confirm(
    ...
    googleLoginBtns.forEach(btn => {
      if (btn.dataset.authClickBound !== '1') {
        btn.dataset.authClickBound = '1';
    ...
    if (window.AuthManager && typeof window.AuthManager.registerUniqueAuthHandler === 'function') {
      window.AuthManager.registerUniqueAuthHandler('authUiSync', user => {
        void syncAuthUiState(user);
      });
    }

    try {
      if (
        !window.__WFX_AUTH_UI_FALLBACK_LISTENER__ &&
        typeof window.__WFX_AUTH_SUBSCRIBE__ === 'function'
      ) {
        window.__WFX_AUTH_UI_FALLBACK_LISTENER__ = true;
        window.__WFX_AUTH_SUBSCRIBE__(user => {
          void syncAuthUiState(user || null);
        });
      }
    } catch (error) {
      Logger.debug('Could not bind direct auth UI fallback listener', 'AUTH', error);
    }

    window.addEventListener('auth:logout', () => {
      void syncAuthUiState(null);
    });

    try {
      const currentUser =
        typeof window.__WFX_AUTH_CURRENT_USER__ === 'function'
          ? window.__WFX_AUTH_CURRENT_USER__()
          : window.firebase && typeof window.firebase.auth === 'function'
            ? window.firebase.auth().currentUser
            : null;
      if (currentUser) {
        void syncAuthUiState(currentUser);
      }
    } catch (_e) {}

    setTimeout(() => {
      try {
        if (authUiHydrated) return;
        const settledUser =
          typeof window.__WFX_AUTH_CURRENT_USER__ === 'function'
            ? window.__WFX_AUTH_CURRENT_USER__()
            : null;
        void syncAuthUiState(settledUser || null);
      } catch (_e) {}
    }, 1500);
    ...
    AppState.subscribe('user', (newUser, oldUser) => {
      ...
      Logger.debug(`Old user: ${(oldUser && oldUser.email) || 'none'}`, 'AUTH');
      Logger.debug(`New user: ${(newUser && newUser.email) || 'none'}`, 'AUTH');
    ...
    Logger.debug('Authentication listeners successfully initialized', 'AUTH');
  };

  // Export core functions to window for early/unified access
  window.setupAuthListeners = setupAuthListeners;
  window.handleGoogleAuth = (...args) => {
    if (typeof handleGoogleAuth === 'function') {
      return handleGoogleAuth(...args);
    }
    return undefined;
  };
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    ...
    window.addEventListener('DOMContentLoaded', () => setupAuthListeners());
  }
})();
