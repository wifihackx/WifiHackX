...
(function () {
  ...
  let googleDelegationRetryTimer = null;
  const clearGoogleDelegationRetryTimer = () => {
    if (googleDelegationRetryTimer) {
      clearTimeout(googleDelegationRetryTimer);
      googleDelegationRetryTimer = null;
    }
  };
  ...
  const enforceShellVisibilityForView = viewId => {
    ...
    } else if (panel && !header?.classList.contains('mobile-menu-open')) {
  ...
  const setupAuthListeners = (attempt = 0) => {
    ...
    const notifyApiKeyReferrerFix = () => {
      const authDomain =
        window.RUNTIME_CONFIG?.firebase?.authDomain || 'white-caster-466401-g0.firebaseapp.com';
      notify(
    ...
    const ensureAppCheckForAuth = async () => {
      try {
        ...
        if (status.disabled) {
          ...
          if (isLocalDevHost()) {
            const appCheckEnabled = localStorage.getItem('wifihackx:appcheck:enabled') === '1';
            const debugToken = (
    ...
    const registerGoogleDelegationHandlers = (attempt = 0) => {
      ...
      if (delegation && typeof delegation.registerHandler === 'function') {
        ...
        clearGoogleDelegationRetryTimer();
        Logger.debug('Google auth handlers registered with EventDelegation', 'AUTH');
        return;
      }

      if (attempt < 8) {
        googleDelegationRetryTimer = setTimeout(
          () => registerGoogleDelegationHandlers(attempt + 1),
          300
        );
        return;
      }
  ...
  const ensureAuthBindingsAlive = () => {
    try {
      ...
      if (!submitBound || !clickBound) {
        Logger.debug('Detected unbound login form after template refresh, rebinding...', 'AUTH');
  ...
  let authBindingsHeartbeatTimer = null;
  const startAuthBindingsHeartbeat = () => {
    if (authBindingsHeartbeatTimer) return;
    let attempts = 0;
    authBindingsHeartbeatTimer = setInterval(() => {
      attempts += 1;
      const isStable = ensureAuthBindingsAlive();
      if (isStable || attempts >= 20) {
        clearInterval(authBindingsHeartbeatTimer);
        authBindingsHeartbeatTimer = null;
      }
    }, 1500);
  };
  ...
  globalThis.getAuthBindingStatus = () => {
    ...
    const loginSubmitBtn = loginForm
      ? loginForm.querySelector('button[data-testid="login-submit"], button[type="submit"]')
      : null;
    const registerForm = document.getElementById('registerFormElement');
    const resetForm =
    ...
    return {
      ...
      forms: {
        login: {
          ...
          clickBound: loginSubmitBtn ? loginSubmitBtn.dataset.authClickBound === '1' : false,
        },
        register: {
          present: !!registerForm,
          submitBound: registerForm ? registerForm.dataset.authSubmitBound === '1' : false,
        },
        resetPassword: {
          present: !!resetForm,
  ...
  const setupAuthStateObserver = async () => {
    ...
    Logger.debug('Configurando observador de estado vía AuthManager...', 'AUTH');

    window.AuthManager.registerUniqueAuthHandler('auth_js_observer', async user => {
      Logger.info(
        user ? `Auth state changed: ${user.email}` : 'Auth state changed: No user',
        'AUTH'
      );
      ...
      try {
        if (user) {
          ...
          const hasCachedAdmin = previousUser.uid === user.uid && previousUser.isAdmin === true;
            ...
            try {
              ...
              if (isAdmin) {
                ...
                try {
                  ...
                  Logger.debug('No se pudo guardar isAdmin en localStorage', 'AUTH', e);
                }
              } else {
                try {
                  localStorage.removeItem('isAdmin');
          ...
          if (loginView) {
            ...
            if (document.body && document.body.getAttribute('data-current-view') === 'loginView') {
              showHomeViewSafe();
            }
          }

          // Defensive UI restore after auth transitions:
          // prevents header/buttons from staying hidden until manual refresh.
          restoreHeaderFooter('auth observer');
        } else {
          ...
          // Mantener shell coherente cuando la sesión expira/cambia
          // fuera del botón logout (refresh token, otra pestaña, etc).
          const currentView =
            document.body?.getAttribute('data-current-view') ||
            localStorage.getItem('currentView') ||
            'loginView';
          enforceShellVisibilityForView(currentView);
        }
      } catch (error) {
  ...
  if (!globalThis.__AUTH_SHELL_ENFORCER_INITED__) {
    globalThis.__AUTH_SHELL_ENFORCER_INITED__ = true;

    const applyCurrent = () => {
      const currentView = document.body?.getAttribute('data-current-view') || 'homeView';
      enforceShellVisibilityForView(currentView);
    };
    ...
    const observer = new MutationObserver(mutations => {
      for (const mutation of mutations) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'data-current-view') {
          applyCurrent();
          break;
        }
      }
    ...
    const mountObserver = () => {
      const bodyEl = targetBody();
      if (!bodyEl) return;
      observer.observe(bodyEl, { attributes: true, attributeFilter: ['data-current-view'] });
      applyCurrent();
    };
    ...
    window.addEventListener('resize', applyCurrent, { passive: true });
  }
  ...
  document.addEventListener(
    ...
    e => {
      ...
      if (target.closest('[data-action="showLoginView"]')) {
        setTimeout(() => {
          const currentLoginForm = document.getElementById('loginFormElement');
          if (currentLoginForm) {
            currentLoginForm.reset();
  ...
  document.addEventListener(
    ...
    () => {
      if (document.visibilityState === 'visible') {
        scheduleEnsureAuthBindings(false);
      }
    },
    true
  );
