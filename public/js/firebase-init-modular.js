...
const firebaseConfig = {
  apiKey: 'AIzaSyAdNsbQgvBae9PIzCTLIEQ6P5SU7rFNfsw',
  authDomain: 'white-caster-466401-g0.firebaseapp.com',
  projectId: 'white-caster-466401-g0',
  storageBucket: 'white-caster-466401-g0.firebasestorage.app',
  messagingSenderId: '304065367414',
  appId: '1:304065367414:web:94ffd6a318ee4b6c327e57',
...
async function initFirebase() {
  ...
  logDebug('Loading modular Firebase SDK...', 'FIREBASE');

  const appMod = await loadFirebaseModule('app');
  const authMod = await loadFirebaseModule('auth');
  const firestoreMod = await loadFirebaseModule('firestore');
  const storageMod = await loadFirebaseModule('storage');
  const functionsMod = await loadFirebaseModule('functions');
  ...
  const functions = getFunctions(app, 'us-central1'); // Especificar región explícitamente
...
window.getAdminClaims = async function (user, forceRefresh = false) {
  ...
  try {
    ...
    window.__adminClaimsCache = { uid: user.uid, claims, ts: now };
    return claims;
  } catch (error) {
