...
function setupAdminDashboardData() {
  ...
  proto.loadUsersCountFromAuth = async function () {
    try {
      log.info(
        'Obteniendo conteo de usuarios desde Firebase Auth...',
        CAT.ADMIN
      );

      const getUsersCountFunction = firebase
        .functions()
        .httpsCallable('getUsersCount');
      const result = await getUsersCountFunction();

      if (result.data && result.data.success) {
        const count = result.data.count;
        log.info(`✅ Usuarios en Firebase Auth: ${count}`, CAT.ADMIN);
        this.updateUsersCountUI(count);
      } else {
        log.warn(
          'Cloud Function getUsersCount no devolvió datos válidos',
          CAT.ADMIN
        );
      }
    } catch (error) {
