...
function setupCommonHandlers() {
  ...
  window.EventDelegation.registerHandler('showCart', (element, event) => {
    ...
    setTimeout(() => {
      ...
      if (checkoutBtn) {
        ...
        console.log(
          '[ICON-FIX-V4] Icono SVG shopping-bag actualizado en showCart'
        );
      }
    }, 50);
  ...
  window.EventDelegation.registerHandler('checkout', (element, event) => {
    ...
    checkoutBtn.innerHTML =
      '<i data-lucide="loader" aria-hidden="true" class="spinning"></i> Procesando...';
   ...
   */
  window.EventDelegation.registerHandler('showLoginView', (element, event) => {
    if (event) {
      event.preventDefault();
    }
  ...
  window.EventDelegation.registerHandler('logout', async (element, event) => {
    ...
    try {
      ...
      // ✅ FIX: Limpiar carrito al hacer logout para evitar que otros usuarios vean productos de otros
      if (
        window.CartManager &&
      ...
      // ✅ FIX: Limpiar claves de carrito específicas del usuario
  ...
  window.EventDelegation.registerHandler(
    ...
    async (element, event) => {
      ...
      const tryOpenAdmin = (attempts = 0) => {
        ...
        // Fallback: intentar cargar admin-navigation-unified.js manualmente
        if (
          !document.querySelector('script[src*="admin-navigation-unified.js"]')
        ) {
          console.log('[CommonHandlers] 🔧 Loading admin-navigation-unified.js...');
          const script = document.createElement('script');
          script.src = 'js/admin-navigation-unified.js?v=1.2';
          const nonce = window.SECURITY_NONCE || window.NONCE;
          if (nonce) {
            script.nonce = nonce;
          }
          script.onload = () => {
            console.log('[CommonHandlers] ✅ admin-navigation-unified.js loaded');
            if (typeof window.showAdminView === 'function') {
              window.showAdminView();
            }
          };
